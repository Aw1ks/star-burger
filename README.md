...
Определите переменную окружения `SECRET_KEY`, `YANDEX_API_KEY`(API Яндекс-геокодера) - можон получить в [кабинете разработчика](https://developer.tech.yandex.ru/services). Создать файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
