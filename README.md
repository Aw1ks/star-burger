...
Определите переменную окружения `SECRET_KEY`, `YANDEX_API_KEY`(API Яндекс-геокодера) - можно получить в [кабинете разработчика](https://developer.tech.yandex.ru/services). Создать файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
SECRET_KEY=django-insecure-0if40nf4nf93n4
YANDEX_API_KEY=(полученный API ключ)
```
